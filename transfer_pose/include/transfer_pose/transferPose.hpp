...
private:
    ...
    // 创建一个TransformStamped消息来表示坐标变换
    // geometry_msgs::TransformStamped transform_world2pose_;
...
void transferPose::imageCallback(const sensor_msgs::Image::ConstPtr &msg)
{
    ...
    if (msg->header.seq == 0)
    {
        ...
        // 设置transform
        transformStamped.header.stamp = ros::Time::now(); // 设置时间戳
        transformStamped.header.frame_id = "world";       // 设置世界坐标系
        transformStamped.child_frame_id = "pose1";        // 设置pose1为子坐标系

        // 将pose1的位姿信息转换为坐标变换矩阵
        transformStamped.transform.translation.x = v_path_pose[0].pose.position.x;
        transformStamped.transform.translation.y = v_path_pose[0].pose.position.y;
        transformStamped.transform.translation.z = v_path_pose[0].pose.position.z;
        transformStamped.transform.rotation = v_path_pose[0].pose.orientation;
    }
    ...
    if (init_flag_)
    {
        ...
        tf2::doTransform(v_path_pose[msg->header.seq], pose2_in_pose1_frame, transformStamped);
